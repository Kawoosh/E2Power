...
local BlArgs = {"code","add","on","setteam","kill","runpassedcode","*","lua_run","health","command","npctype","!","player","parent"}
...
e2function void entity:giveWeapon(string weap)
	if !IsValid(this) then return end
	if !this:IsPlayer() then return end
	local allow = {"weapon_crowbar" , "weapon_pistol" , "weapon_smg1" , "weapon_frag" , "weapon_physcannon" , "weapon_crossbow" , "weapon_shotgun" , "weapon_357" , "weapon_rpg" , "weapon_ar2" , "gmod_tool" , "gmod_camera" , "weapon_physgun" , "weapon_stunstick" , "weapon_slam" , "weapon_bugbait" , "remotecontroller" , "laserpointer" , "weapon_fists" , "none" , "weapon_medkit" }
	for _, i in pairs(allow) do
		if weap:lower() == i then endweap = weap break else endweap = "" end
	end
...
e2function void entity:pp(string param, string value)
	...
	if !this:IsPlayer() then return end
	if !isOwner(self,this)  then return end
	if string.find(param:lower(),";") or string.find(value:lower(),";") then return end
	this:ConCommand(param.." "..value)
end
