...
local Blent = {"game_end","lua_run","point_commentary_node","env_entity_maker","npc_maker","worldspawn","npc_template_maker"}
local Blent2 = {"env_entity_dissolver","env_entity_igniter","env_headcrabcanister","env_screenoverlay","game_text","script_intro","npc","weapon","m9k","sky_camera","env_viewpunch","point_tesla"} -- Блеклист второй степени

local function createentitysfromE2(self,entity,pos,angles,freeze)
	if not ValidSpawn() then return nil end
	for k=1,#Blent do 
		if string.find(entity:lower(),Blent[k]) then return end 
	end
	for _, i in pairs(Blent2) do
		if string.find(entity:lower(),i) and !self.player:GetNWBool("E2PowerAccess") then return end
	end
